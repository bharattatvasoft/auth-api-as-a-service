...
import com.waseel.dawy.repository.RequestTrackingRepo;
...
public class RequestAndResponseLoggingFilter extends OncePerRequestFilter {
    ...
    @Autowired
    private RequestTrackingRepo requestTrackingRepo;
    ...
    @Value("${dawy-api.request.response.logging}")
    ...
    protected void doFilterWrapped(ContentCachingRequestWrapper request, ContentCachingResponseWrapper response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            if (request.getRequestURI().equals("/api/v1/")) {
                RequestTrackingEntity requestTrackingEntity = new RequestTrackingEntity();
                requestTrackingEntity.setMethodType(request.getMethod().toString());
                requestTrackingEntity.setSourceAppId(request.getHeader("sourceAppId"));
                requestTrackingEntity.setURL(request.getRequestURI());
                requestTrackingEntity.setRemoteAddress(request.getRemoteAddr());
                byte[] content = request.getContentAsByteArray();
                String contentString = new String(content, request.getCharacterEncoding());
                requestTrackingEntity.setRequest(contentString);
                byte[] responseContent = response.getContentAsByteArray();
                String responseContentString = new String(responseContent, response.getCharacterEncoding());
                requestTrackingEntity.setResponse(responseContentString);

                requestTrackingRepo.save(requestTrackingEntity);
            }
