...
public class FixedDepositException extends RuntimeException {
	public FixedDepositException(ResponseMessage failedGenerateFd) {
			this.code = failedGenerateFd.getCode();
	        this.message = failedGenerateFd.getMessage();
	        this.status = failedGenerateFd.getStatus();
    }

    public FixedDepositException(String code, String message, String status) {
        this.code = code;
